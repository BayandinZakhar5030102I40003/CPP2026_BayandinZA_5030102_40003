...
BonusRepaint::BonusRepaint(GemColor source)
    : Gem(GemColor::RED), sourceColor(source) {}

void BonusRepaint::activate(
    std::vector<std::vector<std::unique_ptr<Gem>>>& grid, int y, int x,
    std::mt19937& rng) {
  ...
  grid[y][x] = std::make_unique<Gem>(color);
  ...
  for (int i = 0; i < count; i++) {
    ...
    grid[cy][cx] = std::make_unique<Gem>(color);
    grid[cy][cx]->setPosition((float)cx, (float)cy);
    grid[cy][cx]->setTarget(cx, cy);
  }
}

sf::Color BonusRepaint::getColor() const { return sf::Color(255, 255, 255); }
...
std::unique_ptr<Gem> GemFabric::createRandomGem() {
  ...
  return std::make_unique<Gem>(color);
}

std::unique_ptr<Gem> GemFabric::createBonus(GemColor sourceColor) {
  if (rng() % 2 == 0) {
    return std::make_unique<BonusRepaint>(sourceColor);
  } else {
