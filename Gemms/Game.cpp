...
int GemsGame::cntBonus() {
  ...
  for (int y = 0; y < HEIGHT; y++) {
    for (int x = 0; x < WIDTH; x++) {
      if (!grid[y][x]->isEmpty() && typeid(*grid[y][x]) != typeid(Gem) &&
          typeid(*grid[y][x]) != typeid(EmptyCell)) {
        cnt++;
      }
    }
  }
      ...
      for (int dir = 0; dir < 4; dir++) {
        ...
        if (line.size() >= 3) {
          ...
          if (beforeY >= 0 && beforeY < HEIGHT && beforeX >= 0 &&
              beforeX < WIDTH &&
              typeid(*grid[beforeY][beforeX]) != typeid(Gem)) {
          ...
          if (afterY >= 0 && afterY < HEIGHT && afterX >= 0 && afterX < WIDTH &&
              !grid[afterY][afterX]->isEmpty() &&
              typeid(*grid[afterY][afterX]) != typeid(Gem)) {
            bool alreadyAdded = false;
            for (auto& bonus : bonusesToActivate) {
...
void GemsGame::swapGems(int x1, int y1, int x2, int y2) {
  ...
  if (typeid(*grid[y1][x1]) != typeid(Gem) ||
      typeid(*grid[y2][x2]) != typeid(Gem))
    return;
